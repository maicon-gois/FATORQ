...
import { Check, ExternalLink, Eye, MessageCircle, X } from 'lucide-react';
...
function PreviewImage({ src, alt, className }: { src: string; alt: string; className?: string }) {
  ...
  if (failed) {
    return (
      <div
        className={cn(
          'flex items-center justify-center bg-gradient-to-br from-cyan-950/40 to-slate-900 text-slate-500 text-sm',
          className
        )}
      >
        Adicione o print em {src}
      </div>
    );
  }
